...
ACTION_DISPLAY= (
    # L - long, S - short, D-divd, U - update
    ('L', '买入'),
    ('S', '卖出'),
    ('D', '分红'),
    ('U', '更新')
...
class StockCashFlow(models.Model):
    ...
    action = models.CharField(db_column='Action', max_length=10, choices=ACTION_DISPLAY, null=True)
...
class StockTransDeal(models.Model):
    ...
    #     "divd": [[1709881200000, 价格], [1709881200001, 价格], ...],
    #     "info": [{"stamp": 1709881200000, "deal": [["mm/dd", "L/S/D", 价格/分红金额, 数量], ["mm/dd", "L/S/D", 价格/分红金额, 数量]]}, ...]
    # },
    # "div": {...}
    # }
    day = models.TextField(db_column='Day', blank=True, null=True)
    week = models.TextField(db_column='Week', blank=True, null=True)
...
class StockTransFlow(models.Model):
    ...
    # ################### 本次部分 ################### #
    action = models.CharField(db_column='Action', max_length=10, choices=ACTION_DISPLAY, blank=True, null=True)
    # 成交时的价格（价格永远为正）及数量（买入为正，卖出为负）
    price = models.CharField(db_column='Price', max_length=20, blank=True, null=True)
    qty = models.CharField(db_column='Qty', max_length=20, blank=True, null=True)
    # 成交时的股票市值（与 qty 同符号）,如果 intent == L，代表给出去的现金（正值），反之为收到的现金（为负）
    # 因此计算实际收益时，将所有 amount 加起来，取相反数就是收益
    ...
    # 复权时需要重新计算
    cost = models.CharField(db_column='Cost', max_length=20, blank=True, null=True)
    # 总的股票持仓市值（不包含费用，可正可负，用于计算 StockCashFlow 股票市值），保留 2 位小数，当平仓时（position = 0），stock = 0
    # 复权时需要重新计算
    stock = models.CharField(db_column='Stock', max_length=20, blank=True, null=True)
