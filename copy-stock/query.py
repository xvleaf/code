...
def for_quote_trend(tscode, deci):
    """
    为 get_trend_data 和 get_quote_data 提供分时数据(采用1分钟K线等效）
...
def get_trend_data(tscode, deci):
    ...
    get_data = for_quote_trend(tscode, deci)
    ...
    # 上午交易时段
    am_range = pd.date_range(start=am_start, end=am_end, freq='1T')
    # 下午交易时段
    pm_range = pd.date_range(start=pm_start, end=pm_end, freq='1T')
    # 合并两段索引
...
def get_quote_data(tscode, deci):
    """
    获取实时数据(采用1分钟K线等效）
    :param tscode: str 000333.SZ
    :param deci: int 2或3，保留小数位数
