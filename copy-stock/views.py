...
@login_required
def test(request):
    df = query.get_trend_data('000333.SZ', 2)
    # df = query.get_industry_member('000333.SZ')
    # df = query.get_match_industry('801110.SI')
    # df = query.get_kline_data('E', '000333.SZ', '20260101', '20260725', 'D')
    # df = query.get_index_basic()
    return HttpResponse(df, content_type="text/plain;charset=utf-8")
    ...
    if request.method != 'POST':
        ...
        for each in cash_flow_list:
            ...
            try:
                ...
                # 如果 amount 不为空，则显示 action/amount/profit/fee/risk
                action = trans_data.get_action_display()
    ...
    if request.method != 'POST':
        ...
        # elif request_func == 'calc':
            ...
            try:
                ...
                chance = func.get_deal_chance(intent, float(price), float(target), float(stop))
            except ValueError:
                permit = ''
                chance = ''
...
def trans_view(request, code):
    ...
    if request.method != 'POST':
        ...
        direction = "做多" if float(trans_flow_inst.position) > 0 else "做空"
        ...
        if request_func == 'calc':
            ...
            if target and stop:
                ...
                intent = 'L' if position > 0 else 'S'

                chance = func.get_deal_chance(intent, cost, target, stop)

                risk = func.get_deal_risk(cost, position, stop)
...
def trans_deal(request, code):
    ...
    if request.method != 'POST':
        ...
        data = func.handle_trans_deal(session_key, code, 1)
...
def trans_divd(request, code):
    ...
    if request.method != 'POST':
        ...
        divd_qty = round(abs(float(request.POST.get('qty'))), 2)
        divd_cash = round(abs(float(request.POST.get('cash'))), 2)
        divd_fee = round(float(request.POST.get('fee')), 2)

        trans_flow_inst = StockTransIndex.objects.get(code=code.split('.')[1]).Flows.all().last()
        position_exist = float(trans_flow_inst.position)
        cost_exist = float(trans_flow_inst.cost)
        stock_exist = float(trans_flow_inst.stock)
        target_exist = float(trans_flow_inst.target)
        stop_exist = float(trans_flow_inst.stop)
        ...
        stock_divd = round(stock_exist - divd_cash, 2)
        cost_divd = round((cost_exist * position_exist - divd_cash + divd_fee) / divd_qty, 6)
        cost_gap = cost_divd - cost_exist
        target_divd = round(max(target_exist + cost_gap, 0.01), deci)
        stop_divd = round(max(stop_exist + cost_gap, 0.01), deci)
        chance_divd = func.get_deal_chance('L' if position_exist > 0 else 'S', cost_divd, target_divd, stop_divd)
        risk_divd = func.get_deal_risk(cost_divd, divd_qty, stop_divd)

        try:
            with transaction.atomic():
                trans_exclude_fields = [
                    'flow', '_state', 'index_id', 'deal_id', 'trans_id', 'cash_id', 'comments'
                ]
                trans_fields_dict = {
                    key: value for key, value in vars(trans_flow_inst).items() if key not in trans_exclude_fields
                }
                trans_fields_dict.update({
                    ...
                    'stock': stock_divd,
                    'position': divd_qty,
                    'profit': -divd_fee if divd_fee != 0 else 0,
                    'risk': risk_divd,
                    # 用于process_deal函数
                    'comments': divd_cash
                })
                ...
                cash_flow_inst = StockCashFlow.objects.all().last()
                cash_new = round(float(cash_flow_inst.cash) + divd_cash - divd_fee, 2)
                stock_new = round(float(cash_flow_inst.stock) - divd_cash, 2)
                total_new = round(cash_new + stock_new, 2)
                profit_new = round(float(cash_flow_inst.profit) - divd_fee, 2)
                risk_new = round(float(cash_flow_inst.risk) + risk_divd - risk_exist, 2)
                remain_new = round(float(cash_flow_inst.permit) - risk_new, 2)
                ...
                fetch.Deal.save_deal(code.split('.')[1])
...
def review_list(request, cat):
    ...
    if request.method != 'POST':
        ...
        stock_data = fetch.Stock.data(page_list)

        # 由于page_list里面重复的股票，fetch.Stock.data仅体现一次
        # 因此stock_data内的数据需要重新按page_list顺序整理
        data = []
        for each in page_list:
            deci = 3 if each.type == 'F' else 2
            for item in stock_data:
                if item['code'] == each.code:
                    # 必须复制，否则因为修改了 stock_data 中原字典的引用，导致后面会覆盖之前的 flow 值
                    item_copy = item.copy()
                    item_copy.update({'flow': each.flow, 'deci': deci})
