...
def get_deal_chance(intent: str, price: float, target: float, stop: float, reverse: bool = False):    
    if not target or not stop:
        chance = 0
    else:        
        if reverse:
            intent = 'S' if intent == 'L' else 'L'
...
def get_deal_risk(price: float, qty: float, stop: float):
    risk = round(max(math.ceil((price - stop) * qty), 0), 2) if stop != -1 else ''
    return risk
...
# 不包含交易费用
def get_deal_profit(code: str, qty: float, price: float, cal_fee: bool = False):
    trans_flow_list = StockTransIndex.objects.get(code=code).Flows.all()
    amount = 0    
    fee = 0
    ...
    profit = -1 * round(qty * price + amount + fee, 2)
    return profit
...
def handle_trans_deal(session_key: str, code: str, kind: int = 0, intent: str = None, values: dict = None):
    ...
    if focus_index_inst:
        ...
        which_inst = 'trans'
        flow_inst = trans_index_inst.Flows.all().last()
        intent = values.get('intent') if values else (intent if intent else (
            ('L' if float(flow_inst.position) > 0 else 'S') if kind == 0 else (
            'S' if float(flow_inst.position) > 0 else 'L')))
        deal_cat = 'deal'
    ...
    stock_exist = float(getattr(flow_inst, 'stock', 0))
    ...
    if values:
        ...
        price = float(getattr(flow_inst, 'cost', flow_inst.price))

        qty_str = flow_inst.qty if which_inst == 'focus' else flow_inst.position
        qty = abs(float(qty_str)) * (1 if intent == 'L' else -1)
    ...
    if position == 0:
        ...
        this_risk = 0
        this_fee = fee
        # 由于每次交易，手续费均计入收益，因此平仓时不计算前序交易手续费
        this_profit = round(get_deal_profit(code, this_qty, price, False) - fee, 2)
    ...
    # 交易方向与原交易方向相同，或未全部平仓，或全新交易
        ...
        this_amount = round(price * qty, 2)
        this_cost = get_deal_cost(code, price, qty, fee)
        this_profit = round(-1 * fee, 2)

        # 全新交易
        if position_exist == 0:
            this_stock = price * qty
        elif position_exist > 0:
            # 若交易与交易方向一致，则使用当前价格；若相反，则按百分比折减
            this_stock = ((price * qty) + stock_exist) if position > position_exist else (stock_exist * (position / position_exist))
        else:
            this_stock = ((price * qty) + stock_exist) if position < position_exist else (stock_exist * (position / position_exist))
        this_stock = round(this_stock, 2)

        this_target = float(values.get('target')) if values and values.get('target') else float(flow_inst.target)
        this_stop = float(values.get('stop')) if values and values.get('stop') else float(flow_inst.stop)

        if position >= 0:
            reverse = True if intent == 'S' else False
        else:
            reverse = True if intent == 'L' else False

        this_chance = get_deal_chance(intent, this_cost, this_target, this_stop, reverse) if this_target and this_stop else ''
        this_risk = get_deal_risk(this_cost, position, this_stop) if this_stop else ''
        this_fee = fee
        guide = {
            'cost': this_cost,
            'profit': this_profit,
    ...
    # 平仓掉原交易，并反向建仓
        ...
        next_cost = price
        next_stock = round(price * next_qty, 2)
        next_target = float(values.get('target')) if values and values.get('target') else ''
        next_stop = float(values.get('stop')) if values and values.get('stop') else ''
        next_chance = get_deal_chance(intent, next_cost, next_target, next_stop) if next_target and next_stop else ''
        next_risk = get_deal_risk(next_cost, next_position, next_stop) if next_stop else ''
        next_fee = 0
        next_profit = 0
        guide = {
            'cost': next_cost,
    ...
    # elif kind == 2:
        ...
        if index_ohlc != -1:
            ...
            # 需要提前得到，因为trans_flow数据库先执行写入
            if status == 0 or status == -1:
                # 计算复盘收益时，需要将前序交易手续费计算在内
                review_profit = round(get_deal_profit(code, this_qty, price, True) - fee, 2)
                ...
                try:
                    with transaction.atomic():
                        ...
                        trans_flow_inst = StockTransFlow.objects.create(
                            ...
                            stock=this_stock if each == 'this' else next_stock,
                        ...
                        if each == 'this':
                            ...
                            if status == 0 or status == -1:
                                ...
                                trans_review_inst.profit = review_profit
                        ...
                        trans_flow_inst.deal = StockTransDeal.objects.get(code=code)
                        ...
                        if each == 'this':
                            ...
                            each_qty = this_qty
                            each_fee = this_fee
                            stock_gap = this_stock - stock_exist
                        ...
                        total_permit = round(float(cash_flow_inst_last.permit), 2)
                        total_risk = round(float(cash_flow_inst_last.risk) + risk_gap, 2)
                        total_remain = round(total_permit - total_risk, 2)
