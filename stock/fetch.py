...
class Deal:
    ...
    def process_deal(trans_flow_list: List[Dict], right: str, period: str, market_with_code: str) -> dict:
        ...
        # 既有买入也有卖出集合，数组格式为[时间戳，最低价 * 0.98]
        dual = []
        # info 用于K线指示, 格式为[{'stamp':ohlc[0]日期戳，'deal':[买入日期(mm/dd)，交易方向:'L'-买入/'S'-卖出/'D'-分红，价格，数量]}]
        info = []
        # 买入集合，数组格式为[时间戳，最低价 * 0.98]
        long = []
        # 卖出集合，数组格式为[时间戳，最高价 * 1.02]
        short = []
        # 分红集合，数组格式为[时间戳，最高价 * 1.02]
        divd = []

        # check 的原因是交易当天的周/月 k 线不是周/月的最后一个交易日，且最高或最低股价不正确
        # check 用于后面 js 调用 deal 数据库时，核查是否 long/short/dual 最后一个数据是否与 trans_flow_list 最后交易日期进行过核查
        # check 将在get_deal函数中更新
        ...
        for each in index:
            if each[0] >= 0:
                ...
                elif 'S' in group_trade:
                    short.append([ohlc_each[0], round(short_factor * ohlc_each[2], 3)])
                else:
                    divd.append([ohlc_each[0], round(divd_factor * ohlc_each[2], 3)])

        data = {right: {'check': check, 'info': info, 'dual': dual, 'long': long, 'short': short, 'divd': divd}}
        return data
    ...
    def save_deal(code: str) -> None:        
        ...
        trans_flow_list = StockTransFlow.objects.filter(code=code).values(
            'market', 'name', 'date', 'action', 'price', 'qty', 'position', 'comments'
        ).filter(Q(event='T') | Q(event='D') | Q(event='E')).order_by('flow')
    ...
    def get_deal(ohlc: list, market_with_code: str, right: str, period: str):
        try:
            ...
            if right == 'adj':
                # 设置为在 5 秒内 k 线图中，点击 div 再点击 adj，代表更新交易的复权数据
                if confirm_adj_deal_data and confirm_adj_deal_data == [code, 'div', period]:
                    ...
                    cache.set(confirm_adj_deal_data_cache_key, [code, 'adj', period], 5)
            else:
            ...
            try:            
                ...
                if deal_check['done'] == 0:
                    ...
                    for r in ['adj', 'div']:
                        ...
                        for kind in ['dual', 'long', 'short', 'divd']:
