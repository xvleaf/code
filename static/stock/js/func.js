...
function deal_intent_changed(deal_cat, code) {
	...
	if (data) {
		...
		if (deal_cat === 'plus') {
			...
			const under_value = data['under'];
			const type_value = data['type'];

			if (under_value === 'SH') {
				under_value = '上海';
			} else if (under_value === 'SZ') {
				under_value = '深圳';
			} else {
				under_value = '北京';
			}
...
function deal_submit(market_with_code) {
	...
	layer.confirm('<p style="text-align:center">税费是否正确？</p>', {
		title: '确认',
		btnAlign: 'c', //按钮居中
		btn: ['是', '否'],
		shade: 0.5 //遮罩透明度
...
function divd_submit(market_with_code) {	
	...
	layer.confirm('<p style="text-align:center">填报是否正确？</p>', {
		title: '确认',
		btnAlign: 'c', //按钮居中
		btn: ['是', '否'],
		shade: 0.5 //遮罩透明度
