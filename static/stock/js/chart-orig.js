			...
			formatter: function () {
				...
				if (deal !== '') {
					...
					if (deal['amount'].toString() !== '') {
						html += `<tr><td>事件 ${deal['event']}</td><td class="pad-left-1">方向 ${deal['action']}</td></tr>
...
async function trend_init(cat, market_with_code, trend_act) {
    trends_index = 0;
    trends_index_new = 0;
    set_trend_act(trend_act);
    
    // 并行执行两个异步操作
    const [quoteResult, trendResult] = await Promise.all([
        get_quote_data(cat, market_with_code),
        get_trend_data(cat, market_with_code)
    ]);
    
    // 只有当 trend 数据成功获取时才创建趋势图
    if (trendResult) {
        create_trend();
    }
}
...
async function get_quote_data(cat, market_with_code) {
	let url = "chart/data";
	let post = {"site": site, "cat": cat, "code": market_with_code, "func": "quote"};
	let data = await ajax_async(url, post);
	if (data) {
		set_quote_data(data);
	}
}
...
async function get_trend_data(cat, market_with_code) {
	let status = true;
				...
				if (deal['info']) {
					...
					if (j !== -1) {
						...
						for (let e of each) {
							let display;
							if (e[1] === 'L') {
								display = "买入 " + e[0] + ' ' + e[2] + '*' + e[3]
							} else if (e[1] === 'S') {
								display = "卖出 " + e[0] + ' ' + e[2] + '*' + e[3]
							} else{
								display ="分红 " + e[0] + ' ' + e[2] + ',' + e[3]
							}
...
function create_kline_extra(cat, market_with_code) {
	if (get_kline_data(cat, market_with_code, 'extra')) {
		...
		klines.addSeries(series_long);
		klines.addSeries(series_short);
		klines.addSeries(series_dual);
		klines.addSeries(series_divd);
	}
}
